"""
Viral Sweet Potato Content Breakdown — 一键运行入口
...
def main():
    parser = argparse.ArgumentParser(
        description="Viral Sweet Potato Content Breakdown — 一键运行",
...
示例：
  ...
  python run.py "蔡不菜（AI版）" --skip-env
    ...
    print()
    print("🚀 Viral Sweet Potato Content Breakdown — 一键运行")
    ...
    try:
        ...
        print(f"\n📊 Phase 3.6: 数据可视化面板...")
        dashboard_path = generate_dashboard(args.data_dir, args.output_dir)
        if dashboard_path:
            print(f"   ✅ 可视化面板: {dashboard_path}")
    except Exception as e:
        print(f"   ⚠️ 可视化面板生成失败: {e}")
