...
# 主流程
# ----------------------------------------------------------
def crawl_blogger(keyword=None, user_id=None, output_dir=None, port=18060, is_self=False, extra_keywords=None, max_notes=None):
    """
    完整爬取一个博主的全量数据。
    ...
    Args:
        ...
        max_notes: 最多爬取笔记数（None=全量），用于快速测试
    ...
    # 限制笔记数
    notes_for_details = notes
    if max_notes and len(notes) > max_notes:
        sorted_notes = sorted(notes.items(), key=lambda x: x[1].get("likedCount", 0), reverse=True)
        notes_for_details = dict(sorted_notes[:max_notes])
        print(f"\n⚡ 限制模式：仅爬取赞数最高的 {max_notes}/{len(notes)} 条笔记")

    # 获取全部详情
    details = get_all_details(client, notes_for_details, output_dir, nickname)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-notes", type=int, default=None, help="最多爬取笔记数（默认全量），用于快速测试")
