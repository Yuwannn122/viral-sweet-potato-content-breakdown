"""
Viral Sweet Potato Content Breakdown — 自动安装脚本
检测宿主AI平台，将 Skill 复制到对应的 skills 目录。

支持平台：
  - WorkBuddy (CodeBuddy): ~/.workbuddy/skills/viral-sweet-potato-content-breakdown/
  - Claude Code:            ~/.claude/skills/viral-sweet-potato-content-breakdown/
...
SKILL_NAME = "viral-sweet-potato-content-breakdown"
...
def main():
    parser = argparse.ArgumentParser(
        description="Viral Sweet Potato Content Breakdown — 自动安装",
...
示例：
  ...
  python install.py --dry-run          # 仅预览
    ...
    print()
    print("=" * 60)
    print("📦 Viral Sweet Potato Content Breakdown — 安装向导")
    ...
    if args.dry_run:
        ...
        if platform_name == "workbuddy":
            print("  在 WorkBuddy 中直接说「拆解博主 XX」或「拆解这条」即可触发")
        elif platform_name == "claude":
            print("  在 Claude Code 中说「拆解博主 XX」（博主拆解）或「拆解这条」（单条爆款拆解）")
        else:
